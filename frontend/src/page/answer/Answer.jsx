    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://forumserver.sisaytech.com/api/answers/getallanswer/${questionid}`
        );
        ...
        setAnswers(combinedAnswers); // Set state with sorted answers
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `https://forumserver.sisaytech.com/api/answers/postanswer/${questionid}`,
        {
          answer: answerValue,
          userid: user.userid,
        }
      ...
      answerDom.current.value = ""; // Clear textarea after submission
        ...
        <div className={style.answer_form}>
          <form onSubmit={handleSubmit}>
            <textarea
              ref={answerDom}
              onChange={() =>
                setIsTextAreaEmpty(answerDom.current.value === "")
              }
              rows="7"
